...
void Light::SetShaderMtrx(Shader* shader, size_t number) {
    shader->Use();

    shader->Disuse();
}

void DirectionalLight::SetShaderMtrx(Shader* shader, size_t number) {
    shader->Use();
    std::stringstream s;
    ...
    glm::vec3 lightPosition =  -this->direction * 50.f;
